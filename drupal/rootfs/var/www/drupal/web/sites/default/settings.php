...
/* $databases['default']['default'] = [
  ...
  'host' => file_get_contents($path . 'DRUPAL_DEFAULT_DB_HOST'),
  'port' => file_get_contents($path . 'DRUPAL_DEFAULT_DB_PORT'),
  'prefix' => '',
  'driver' => 'mysql',
...
$databases['default']['default'] = [
  ...
  'host'      => getenv('DB_MYSQL_HOST'),  // Maps to your UMaryland server
  'port'      => getenv('DB_MYSQL_PORT') ?: '3306', // Maps to 3306
  'prefix'    => '',
  'driver'    => 'mysql',
  'namespace' => 'Drupal\\Core\\Database\\Driver\\mysql',
];
...
// Read password from secret file or environment variable
$db_password = '';
if (file_exists('/var/run/secrets/DRUPAL_DEFAULT_DB_PASSWORD')) {
    $db_password = trim(file_get_contents('/var/run/secrets/DRUPAL_DEFAULT_DB_PASSWORD'));
} elseif (file_exists($path . 'DRUPAL_DEFAULT_DB_PASSWORD')) {
    $db_password = trim(file_get_contents($path . 'DRUPAL_DEFAULT_DB_PASSWORD'));
} else {
    $db_password = getenv('DRUPAL_DEFAULT_DB_PASSWORD') ?: '';
}

$databases['default']['default'] = [
  'database'  => getenv('DRUPAL_DEFAULT_DB_NAME') ?: (file_exists($path . 'DRUPAL_DEFAULT_DB_NAME') ? trim(file_get_contents($path . 'DRUPAL_DEFAULT_DB_NAME')) : ''),
  'username'  => getenv('DRUPAL_DEFAULT_DB_USER') ?: (file_exists($path . 'DRUPAL_DEFAULT_DB_USER') ? trim(file_get_contents($path . 'DRUPAL_DEFAULT_DB_USER')) : ''),
  'password'  => $db_password,
  'host'      => getenv('DB_MYSQL_HOST') ?: (file_exists($path . 'DB_MYSQL_HOST') ? trim(file_get_contents($path . 'DB_MYSQL_HOST')) : '127.0.0.1'),
  'port'      => getenv('DB_MYSQL_PORT') ?: (file_exists($path . 'DB_MYSQL_PORT') ? trim(file_get_contents($path . 'DB_MYSQL_PORT')) : '3306'),
  'prefix'    => '',
  'driver'    => 'mysql',
